...
$items = mysqli_query($conn, "
    SELECT i.*, p.product_name, p.barcode, p.selling_price, p.cost_price, p.units_per_box, p.cost_per_box, p.description,
           ...
           (SELECT status FROM transfer_requests tr WHERE tr.product_id = i.product_id AND tr.status = 'Pending Warehouse' LIMIT 1) as pending_status
    FROM inventory i
    JOIN products p ON i.product_id = p.product_id
    LEFT JOIN categories c ON p.category_id = c.category_id
        ...
        <div class="page-card">
            ...
            <div class="d-flex align-items-center gap-3 p-3 rounded mb-2 low-stock-item"
                ...
                <div class="text-end">
                    ...
                    <?php if (!empty($r['pending_status'])): ?>
                        <span class="badge bg-info text-dark" style="font-size:9px;"><i class="bi bi-clock-history me-1"></i>PENDING</span>
                    <?php elseif ($isOut): ?>
        ...
        <div id="lsDetailPanel" style="display:none;">
            ...
            <div class="page-card mb-3">
                ...
                <div class="page-card border-warning border-2" style="border:2px solid #f59e0b!important;">
                    ...
                    <button id="generate_btn" class="btn btn-warning text-dark w-100 fw-bold" onclick="generateReport()">
...
function selectProduct(id) {
    ...
    if (detailBody) {
        detailBody.innerHTML = `
            <div class="row g-2 mb-3">
                ...
                ${p.pending_status ? `<div class="col-12 mt-2"><div class="alert alert-info py-2 mb-0" style="font-size:12px;"><i class="bi bi-info-circle-fill me-2"></i>A request for this item is already <strong>Pending Warehouse</strong>.</div></div>` : ''}
                <div class="col-md-3">
    ...
    if (generateBtn) {
        if (p.pending_status) {
            ...
            generateBtn.innerHTML = `<i class="bi bi-hourglass-split me-2"></i>Request Already Pending`;
        } else {
            generateBtn.disabled = false;
            generateBtn.innerHTML = `<i class="bi bi-eye-fill me-2"></i>Preview Low Stock Report &amp; Send to Warehouse`;
        }
    }
