...
.product-item:hover { border-color: #198754; background: #f0faf4; }
...
.product-item .prod-icon { font-size: 32px; margin-bottom: 6px; }
.product-item .prod-name { font-size: 12px; font-weight: 600; margin-bottom: 4px; line-height: 1.3; }
.product-item .prod-price { font-size: 13px; color: #198754; font-weight: 700; }
.product-item .prod-stock { font-size: 11px; font-weight: 600; margin-top: 3px; }
...
.cart-item-price { font-size: 12px; color: #198754; font-weight: 700; white-space: nowrap; }
.qty-btn {
...
.cart-footer { padding: 14px 16px; border-top: 1px solid #e9ecef; }
...
.total-amount { font-size: 22px; font-weight: 800; color: #198754; }
.empty-cart { text-align: center; padding: 40px 20px; color: #adb5bd; }
.empty-cart i { font-size: 48px; margin-bottom: 10px; display: block; }
</style>
...
<div class="cashier-wrap">
    ...
    <div class="product-panel">
        ...
        <div class="product-grid" id="productGrid">
            ...
            <div class="product-item <?= $p['stock'] <= 0 ? 'out-of-stock' : ''; ?>"
                ...
                <div class="prod-stock" style="color:<?= $p['stock'] > 5 ? '#198754' : ($p['stock'] > 0 ? '#ffc107' : '#dc3545'); ?>">
                    <?php if($p['stock'] > 5){ ?>
                        <i class="bi bi-check-circle-fill"></i> <?= $p['stock']; ?> left
                    <?php } elseif($p['stock'] > 0){ ?>
                        <i class="bi bi-exclamation-triangle-fill"></i> Low: <?= $p['stock']; ?>
                    <?php } else { ?>
    ...
    <div class="cart-panel">
        <div class="cart-header">
            ...
            <button class="btn btn-sm btn-outline-light" onclick="clearCart()" title="Clear">
        ...
        <div class="cart-footer">
            ...
            <div class="total-row mb-3">
                <span class="total-label fw-semibold" style="font-size:15px;">Total</span>
                <span class="total-amount">₱<span id="totalAmount">0.00</span></span>
            </div>

            <div class="mb-2">
                <label class="form-label fw-semibold" style="font-size:13px;">Cash Payment (₱)</label>
...
function addToCart(el){
    ...
    if(existing){
        ...
        cart.push({ product_id:id, name, price, quantity:1, subtotal:price, stock });
    }
    renderCart();
}
...
function filterProducts(){
    ...
    $(".product-item").each(function(){
        ...
        $(this).toggle(name.includes(search) && (!category || cat === String(category)));
    });
}
...
function processSale(){
    ...
    if(payment < total){
        Swal.fire('Insufficient Payment',
            `Payment ₱${payment.toFixed(2)} is less than total ₱${total.toFixed(2)}.`,'warning');
        return;
    }

    Swal.fire({
        title: 'Confirm Sale?',
        html: `Total: <strong>₱${total.toFixed(2)}</strong><br>
               Payment: <strong>₱${payment.toFixed(2)}</strong>`,
        icon: 'question',
