...
<div class="page-card">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
            <a href="jobs.php" target="_blank" class="btn btn-outline-primary btn-sm me-2"
                style="border-radius:8px;font-weight:600;">
                <i class="bi bi-globe me-1"></i>View Public Job Board
            </a>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-primary-subtle text-primary border border-primary-subtle fw-semibold" style="font-size:11px;">
                                    <?= htmlspecialchars($pos['department_name'] ?? 'General'); ?>
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="table-actions text-center">
                                ...
                                <button class="action-btn btn-edit" title="Edit" onclick='editJob(<?= json_encode($pos); ?>)'>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="border:none;border-radius:12px;overflow:hidden;">
            ...
            <div class="modal-body" style="padding:24px;">
                <form id="jobForm">
                    ...
                    <div class="row g-3">
                        ...
                        <!-- Department Selection (Auto Input - Non Editable) -->
                        <div class="col-md-6">
                            <label class="form-label" style="font-size:12px;font-weight:600;color:#374151;">
                                Department <span class="text-danger">*</span>
                            </label>
                            <input type="text" class="form-control" id="departmentNameDisplay" readonly placeholder="Auto-filled based on position..." style="border-radius:8px;font-size:13px;background-color:#f3f4f6;color:#374151;font-weight:600;cursor:not-allowed;">
                            <input type="hidden" id="departmentIdSelect" name="department_id" value="">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control" name="slots" id="slots" required min="1" value="1"
                                style="border-radius:8px;font-size:13px;">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" name="salary_min" id="salaryMin" min="0"
                                step="0.01" value="0" style="border-radius:8px;font-size:13px;">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control" name="salary_max" id="salaryMax" min="0"
                                step="0.01" value="0" style="border-radius:8px;font-size:13px;">
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control" name="requirements" id="requirements" rows="4"
                                placeholder="List the qualifications, responsibilities, and requirements for this position..."
            ...
            <div class="modal-footer" style="border-top:1px solid #f0f0f0;padding:14px 24px;">
                ...
                <button type="button" class="btn btn-primary btn-sm" onclick="submitJob()" id="btnSubmitJob"
                    style="border-radius:8px;font-weight:600;">
            ...
            <div class="modal-body" style="padding:24px;">
                ...
                <div class="row mt-3">
                    <div class="col-md-4">
                        ...
                        <div class="detail-value text-primary fw-bold" id="viewDepartment"></div>
                    </div>
                    <div class="col-md-4">
...
<script>
    ...
    ====================================================*/
    function autoFillJobDepartment(selectEl) {
        if (!selectEl) return;
    ...
    window.goToPositions = goToPositions;
    ...
    function editJob(job) {
        ...
        if (!$('#departmentIdSelect').val()) {
            ...
            $('#departmentNameDisplay').val(job.department_name || 'N/A');
        }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response === 'success') {
                        ...
                        Swal.fire('Duplicate', 'An active job posting for this position already exists.', 'warning');
                    } else {
    ...
    function viewJob(job) {
        ...
        $('#viewDepartment').text(job.department_name || 'General Operations');
    ...
    function deleteJob(positionId, positionName) {
        Swal.fire({
            ...
            html: `
                <p class="mb-2 text-muted" style="font-size:13px;">Select the reason for removing <strong>${positionName}</strong>:</p>
                <div class="text-start mb-3">
                    <label class="form-label fw-bold text-dark" style="font-size:12px;">Reason for Removal <span class="text-danger">*</span></label>
                    <select id="removeJobReasonSelect" class="form-select" style="font-size:13px;">
                        <option value="Position Fulfilled / Slots Filled">Position Fulfilled / Slots Filled</option>
                        <option value="Budget Reallocation / Freeze">Budget Reallocation / Freeze</option>
                        <option value="Role Restructure / Deprecated Position">Role Restructure / Deprecated Position</option>
                        <option value="Duplicate Job Posting">Duplicate Job Posting</option>
                        <option value="Cancelled Hiring Request">Cancelled Hiring Request</option>
            ...
            cancelButtonText: 'Cancel',
            preConfirm: () => {
                const reason = document.getElementById('removeJobReasonSelect').value;
                const pwd = document.getElementById('removeJobPassword').value;
                if (!pwd) {
                    Swal.showValidationMessage('Please enter your account password to confirm removal.');
                    return false;
                }
                return { reason, pwd };
            }
                    ...
                    success: function (response) {
                        ...
                        if (response === 'success') {
                            Swal.fire({
                                ...
                                text: positionName + ' has been removed successfully.',
                                timer: 1500,
                                showConfirmButton: false
                            });
                            setTimeout(() => loadPage('hrms_jobs.php'), 1200);
